<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!-- navigator 영역 -->
	<div class="vf_sub_menu" style="background-color: #FFC107;">
		<div style="width: 15%"><a href="${path}/shop_info.do">상점정보보기</a></div>
		<div style="width: 14%"><a href="${path}/productInsertForm.do">상품등록</a></div>
		<div style="width: 14%"><a href="${path}/productList.do">상품목록</a></div>
		<div style="width: 14%"><a href="order_tabList.do">주문확인</a></div>
		<div style="width: 14%"><a href="refundList.do">환불관리</a></div>
		<div style="width: 14%"><a href="#">정산내역</a></div>
		<div style="width: 15%"><a href="shopQnaList.do">문의글목록</a></div>

	</div>