...
<div class="container">
		...
		<tr>
			<td valign="middle" align="right"
				...
				<c:if test="${sessionScope.id == 'admin'}">
					<a href="${path }/view.do?num=1&pageNum=1">관리자페이지</a> | </c:if>
	...
	<!-- 세션이 있는 경우 --> 
				<c:if test="${!empty sessionScope.m_email }">
					...
					<a href="${path }/productInsertForm.do">입점신청</a> 
					<!--  SHOP매니저 | 입점신청 | 회원가입 | 로그인 | 마이페이지| 장바구니 | 로그아웃 | 관리자페이지  | 이벤트/공지 | 고객센터 -->
			</td>
		</tr>
		<tr>
			<td valign="middle" align="center">
				<!-- 로고 (이미지) --> <a href="${path }/main.do"><img
					src="${path}/images/logo7.PNG" width=150px></a>
			</td>
		</tr>
		<tr>
			<td>
				...
				<div class="vf_header_menu">
					<div>
						<a
							href="${path}/productCategoryList.do?search=cate_large&keyword=과일">
							<c:if test="${keyword == '과일'}">
								<font style="font-size: 20px; font-weight: bold;">과일</font>
							</c:if> <c:if test="${keyword != '과일'}">과일</c:if>
						</a>
					</div>
					<div>
						<a
							href="${path}/productCategoryList.do?search=cate_large&keyword=채소">
							<c:if test="${keyword == '채소'}">
								<font style="font-size: 20px; font-weight: bold;">채소</font>
							</c:if> <c:if test="${keyword != '채소'}">채소</c:if>
						</a>
					</div>
					<div>
						<a
							href="${path}/productCategoryList.do?search=cate_large&keyword=잡곡">
							<c:if test="${keyword == '잡곡'}">
								<font style="font-size: 20px; font-weight: bold;">잡곡</font>
							</c:if> <c:if test="${keyword != '잡곡'}">잡곡</c:if>
						</a>
					</div>
					<div>
						<a href="#" style="">이벤트</a>
					</div>
					<div class="vf_header_menu_search"
						style="width: 225px; padding: 0px;">
						<form action="${path}/productSearchList.do?pageNum=1">
							<input type=hidden name="search" value="p_name"> <input
								type=text name="keyword" class="vf_search">
